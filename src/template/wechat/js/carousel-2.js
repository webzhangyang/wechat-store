...
//模拟AJAX发送成功的回调得到的数据
var img=[
    {'images':'images/ab-brand.jpg','imgName':'#brand',index:0},
    {'images':'images/ab-us.jpg','imgName':'#us',index:1},
    {'images':'images/dt-search.jpg','imgName':'#search',index:2},
];

var img2=[
    {'images':'images/test1.jpg','imgName':'#brand',index:0},
    {'images':'images/ab-us.jpg','imgName':'#us',index:1},
    {'images':'images/dt-search.jpg','imgName':'#search',index:2},
];
...
var carousel={
    ...
    init:function(vessel,target,small,img,index,tab){/* vessel容器 初始函数 参数是想要安放轮播的容器，默认使用ul>li的html结构*/
        ...
        window.onresize=function(){
            ...
            var width=innerWidth>=768?768:innerWidth;
            carousel.elemWidth=width;//把此宽度也传给元素的宽度，避免轮播赋值时宽度不更新
            if(len>0){
                target.width(len*width).children('li').css('width',width);
            }
        }
    },
    ...
    eventFunc:function(tar){
        ...
        if(!!this.small){
            this.small.on("click",'li',function(){
                ...
                console.log(tarindex,nowIndex,len);
    ...
    updateHtml:function(img){
        ...
        this.target.html(html).width(img.length*carousel.elemWidth).children('li').css('width',carousel.elemWidth);//给目标容器赋值,同时设定容器的宽度
        if(!!this.small){
